...
**Projeto:** Chatbot Interativo para Fãs da FURIA Esports (CS2) - Processo Seletivo Engenharia de Software

## 1. Introdução e Visão Geral

Este projeto consiste em um chatbot web interativo desenvolvido como parte do processo seletivo de Engenharia de Software. O objetivo principal é criar uma interface de conversação para fãs da equipe de Counter-Strike 2 (CS2) da FURIA Esports, fornecendo informações relevantes como escalação atual, notícias recentes e permitindo uma conversa geral sobre o time, mantendo a persona de um assistente de fãs dedicado.
...
* **RF08:** (Opcional/Conveniência) O sistema deve tentar abrir a interface web no navegador padrão do usuário ao ser iniciado.
...
* **RNF05 (Segurança):** A chave da API Google AI deve ser gerenciada de forma segura. (Nota: Atualmente está hardcoded em `chat.py`, o que **não é recomendado** para produção - Ver seção Melhorias).
* **RNF06 (Portabilidade):** O backend (Python/Flask) deve ser executável em ambientes que suportem Python 3, conforme especificado pelas dependências. O script `run.sh` sugere um ambiente Linux/Unix-like.
...
```mermaid
graph LR
    A[Usuário] --&gt;|1. Acessa Interface| B(Sistema Chatbot FURIA);
    A --&gt;|2. Envia Mensagem| B;
    B --&gt;|3a. Consulta Escalação (HLTV)| C{Serviço Externo: HLTV};
    B --&gt;|3b. Consulta Notícias (RSS)| D{Serviço Externo: Google News};
    B --&gt;|3c. Processa com LLM| E{Serviço Externo: Google AI API};
    B --&gt;|4. Exibe Resposta| A;

    subgraph Sistema Chatbot FURIA
        direction TB
        F[Frontend (HTML/CSS/JS)] &lt;--&gt; G[Backend (Flask)];
        G --&gt; H(Web Scraping - BS4);
        G --&gt; I(Feed Parser - feedparser);
        G --&gt; J(LLM Client - google-genai);
    end
```
...
* **Orquestração da Lógica do Chat:**
    * **Fluxo Atual:** A lógica em `chat.py` usa condicionais (`if/elif/else`) para direcionar a requisição (Scraping, News, LLM). Funciona para poucos casos, mas pode se tornar complexa com mais funcionalidades.
...
**Observação:** Certifique-se de que a chave da API Google AI em `chat.py` (`AIzaSyAOM2B1asxKdp1SFgid5ALvaCUTA2pqmH4`) seja válida ou substitua pela sua própria chave.

## 6. Pontos de Melhoria e Próximos Passos

* **Gerenciamento de Segredos:** Remover a API Key hardcoded do código (`chat.py`) e carregá-la a partir de variáveis de ambiente ou um arquivo de configuração seguro (`.env`).
...
Espero que este relatório detalhado e o `README.md` estruturado atendam às suas expectativas para o processo seletivo e demonstrem seu conhecimento em Engenharia de Software. Boa sorte!